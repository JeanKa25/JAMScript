...
function show_usage() {
    ...
    ${keyWord(
        "Note: By default jamlist lists all the active local programs and print it before terminating."
    )} 

    --- ${keyWord(
        "jamlist displays the folloing attribute of the running apps:"
    )} 
        ...
        5) TYPE: ${body_2(
            "either one of fog, device or cloud depending on the program's type."
        )}
        7) TMUX-ID: ${body_2(
            "the generic tmux ID used by the program (not including the tag)."
        )}
        8) STATUS: ${body_2(
            "running is only program status now."
        )}
        9) HOST: ${body_2(
            "localHost if program is running locally or IPAddress of the remote machine running the program."
        )}
        10) UP-TIME: ${body_2(
            "the duration of the program running in the format of hour-minute-sec."
        )}
    ...
    --- jamlist [--remote]
    ${body_2(
        `Use this flag to include the remote active apps started by the local machine running the jamlist.`
    )}
    ...
    ${body_2(
        `used --dataStore option to list all the active apps with the given dataStore address.`
    )}

    --- jamlist [--tmuxid==tg-25165]
    ${body_2(
        `used ---tmuxid option to list all the active apps with the given tmuxid.`
    )}

    --- jamlist [--port=3]
    ${body_2(
        `used --port option to list all the active apps running on the given port.`
    )}
    
    --- jamlist [--app==XX3]
    ${body_2(
        `used --app option to list all the active apps with the given app name.`
    )}

    --- jamlist [--prog==X]
    ${body_2(
        `used --prog option to list all the active apps with the given program name.`
    )}
...
function watch(filters) {
    ...
    const updateInfo = debounce(async () => {
        ...
        if (
            ...
            let filteredObj = Object.keys(filters)
    ...
    watcher = chokidar
        .watch(watchList, { persistent: true, ignoreInitial: true })
        .on("all", (event, path) => {
...
function getNodeInfo(root = null) {
    ...
    const nodeInfo = [];
    if (
        !fs.existsSync(`${jamfolder}/ports`) ||
        !fs.existsSync(`${jamfolder}/apps`)
    ) {
        return [];
    }
        ...
        for (let port of appToPortMap.get(app)) {
            if (root) {
                if (fs.existsSync(`${appfolder}/${app}/${port}/root`)) {
                    const rootIP = fs
                        .readFileSync(`${appfolder}/${app}/${port}/root`)
                        .toString()
                        .trim();
                    if (rootIP === root) {
                        const fileNames = {
                        ...
                        if (!fs.existsSync(`${appfolder}/${app}/${port}`)) {
                            throw new Error(
                                "FileDirectory is corrupted. TAKE REQUIRED ACTION"
                            );
                        }
...
function printNodeInfo(info) {
    for (let row of info) {
        ...
        let UPTIME = getUpTime(row["startStamp"]);
        const headerString = `   ${row["app"].padEnd(10)} ${row["prog"].padEnd(
            10
        )} ${("Local:" + row["port"]).padEnd(10)} ${row["dataStore"].padEnd(
            20
        )} ${row["machType"].padEnd(10)} ${row["numCnodes"].padEnd(10)} ${row[
            "tmuxid"
        ].padEnd(10)} ${row["status"].padEnd(10)} ${row["host"].padEnd(
            10
        )} ${UPTIME.padEnd(10)}`;
...
function printHeader() {
    const headerString = `   ${chalk.bold.italic(
        "APP".padEnd(10)
    )} ${chalk.bold.italic("PROGRAM".padEnd(10))} ${chalk.bold.italic(
        "PORT".padEnd(10)
    )} ${chalk.bold.italic("D-STORE".padEnd(20))} ${chalk.bold.italic(
        "TYPE".padEnd(10)
    )} ${chalk.bold.italic("C-NODES".padEnd(10))} ${chalk.bold.italic(
        "TMUX-ID".padEnd(10)
    )} ${chalk.bold.italic("STATUS".padEnd(10))} ${chalk.bold.italic(
        "HOST".padEnd(10)
    )} ${chalk.bold.italic("UP-TIME".padEnd(10))}`;
    console.log(headerString);
